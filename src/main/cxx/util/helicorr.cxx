...
void
write_usage_message (ostream &os, const string &pname)
{
  os << "usage: " << pname
     << " {velocity (m s-1)} [skip] < {source data file} [> {target data "
        "file}]"
     << endl;
}
...
/// @param argv[2] The number of lines to skip at the beginning (optional,
/// default = 0).
/// @return an exit code.
///
/// @remark Usage: helicorr {velocity (m s-1)} [lines to skip] < {source file}
/// [> {target file}]
int
