...
///
/// @tparam T The number type used for quadrature calculations. The quadrature
/// weight and abscissa values are defined with a precision of 48 decimal
/// digits.
template <class T = real> class Integrator
{
public:
  ...
  /// Computes the integral of a function, with the limits supplied as
  /// argument, i.e.
  /// @f[ \int_{a}^{b} f(x) dx @f].
  ///
  ...
  template <class F>
  T
  integrate (const F &f, T a, T b, T accuracy_goal = T (1.0E-6),
             natural max_iteration = 100) const
  {
  ...
  ///
  /// @attention The integrand must converge rapidly (faster than @f$ 1/x @f$)
  /// to zero at infinity, i.e.
  /// @f[ \lim_{x\to\infty} \frac{f(x)}{x} = 0 @f].
  template <class F>
  T
  integrate_positive_infinite (const F &f, T accuracy_goal = T (1.0E-6),
                               natural max_iteration = 100) const
  {
    using std::log;
    ...
    return integrate (
        [&f] (T u) -> T {
          return u > T (0.0) ? f (-log (u)) / u : T (0.0);
  ...
  template <class F>
  T
  integrate_negative_infinite (const F &f, T accuracy_goal = T (1.0E-6),
                               natural max_iteration = 100) const
  {
    return integrate_positive_infinite ([&f] (T x) -> T { return f (-x); },
                                        accuracy_goal, max_iteration);
  }
  ...
  template <class F>
  T
  integrate_infinite (const F &f, T accuracy_goal = T (1.0E-6),
                      natural max_iteration = 100) const
  {
...
private:
  ...
  public:
    ...
    Part (const F &f, T a, T b, Formula p, Formula q)
        : f (f), a (a), b (b), p (p), q (q), c (T (0.5) * (a + b)),
          h (T (0.5) * (b - a)), yl (21), yu (21)
    {
      evaluate ();
    }
  ...
  private:
    ...
    /// Evaluates the integration result of this part using the quadrature
    /// formula supplied as argument.
    ///
...
#pragma omp parallel for reduction(+ : result)
  ...
  template <class P> class Part_Compare
  {
  public:
    /// Compares the absolute error of two parts of a numerical integration.
    ///
    ...
    /// @return @c true, if the absolute error of the first part is less than
    /// that of the other part.
    bool
    operator() (const P *p, const P *q) const
    {
      return p->absolute_error () < q->absolute_error ();
    }
  };

  /// A partition of a numerical integral into a complete set of disjoint
  /// parts.
  ///
  /// @tparam F The integrand type.
  /// @tparam P The part type.
  template <class F, class P> class Partition
  {
  public:
    /// The constructor.
    ///
    ...
    /// @param p The formula with less quadrature points.
    /// @param q The formula with more quadrature points.
    Partition (const F &f, T a, T b, Formula p, Formula q)
        : part_compare (Part_Compare<P> ())
    {
      using std::make_heap;
      using std::push_heap;
    ...
    /// Returns the absolute error of the integration result for this
    /// partition.
    ///
    /// @return the absolute error of the integration result.
    T
  ...
  private:
    /// Removes the part with the largest absolute error of integration from
    /// the partition.
    ///
    ...
    pop_part ()
    {
      ...
      pop_heap (parts.begin (), parts.end (),
                part_compare); // move largest to the end
      P *popped = parts.back ();
...
const T Integrator<T>::xi[] = {
  // abscissas for Q13
  T (0.0000000L), T (0.2500000L), T (0.5000000L), T (0.7500000L),
  T (0.8750000L), T (0.9375000L), T (1.0000000L),
  // additional abscissas for Q19, Q27 and Q41
  T (0.3750000L), T (0.6250000L), T (0.9687500L),
  // additional abscissas for Q27 and Q41
  T (0.1250000L), T (0.6875000L), T (0.8125000L), T (0.9843750L),
  // additional abscissas for Q41
  T (0.1875000L), T (0.3125000L), T (0.4375000L), T (0.5625000L),
  T (0.8437500L), T (0.9062500L), T (0.9921875L)
};
