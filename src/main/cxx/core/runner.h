...
public:
  ...
  run ()
  {
    ...
    if (get_arg_count () != 8)
      {
        throw invalid_argument ("especia::Runner::run() Error: an invalid "
    ...
    const Optimizer::Result result = optimizer.minimize (
        model, model.get_initial_parameter_values (),
        model.get_initial_local_step_sizes (), global_step_size,
        model.get_constraint (), Tracer<> (cout, trace_modulus));
    ...
    model.set (&result.get_parameter_values ()[0],
               &result.get_parameter_uncertainties ()[0]);
    model.put (cout);
...
private:
  ...
  template <class T = real> class Tracer
  {
  public:
    /// The constructor.
    ///
    ...
    Tracer (std::ostream &output_stream, natural modulus,
            natural precision = 4, natural width = 12)
  ...
  void write_result_messages (std::ostream &os,
